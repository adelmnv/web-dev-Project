...
export class TourListComponent implements OnInit {
  ...
  loadInitialData(): void {
    this.tourService
      .getMealTypes()
      .subscribe((data) => (this.mealTypes = data));
  ...
  applyFilters() {
      ...
      duration:
        this.selectedDepartureDate && this.selectedReturnDate
          ? this.calculateDuration(
              new Date(this.selectedDepartureDate),
              new Date(this.selectedReturnDate)
            )
          : undefined,
    ...
    this.loading = true;
    this.tourService
      .findTours(filters.mealTypeId, filters.duration, filters.cityId)
      .subscribe({
        next: (data) => {
